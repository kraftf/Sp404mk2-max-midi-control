...
target = sys.argv[1] if len(sys.argv) > 1 else os.path.join(HERE, '..', 'Roland_SP404MK2_Control-32.maxpat')
...
# 3. varnames unique, and the 40 Bus{1-5}_{param} names match the subscribe list exactly
varnames = {}
...
if sub_msg is None:
    ...
    bg_varnames = {vn for vn in varnames if vn.startswith('Bus')}
...
# 5. sanity counts
n_bg = sum(1 for bid in box_by_id if bid.startswith('obj-pv2-bg-'))
n_hwh = sum(1 for bid in box_by_id if bid.startswith('obj-pv2-hwh-'))
n_sh = sum(1 for bid in box_by_id if bid.startswith('obj-pv2-sh-'))
if n_bg != 40:
    errors.append(f'expected 40 bg client objects, found {n_bg}')
if n_hwh != 40:
    errors.append(f'expected 40 hw-holder objects, found {n_hwh}')
if n_sh != 40:
    errors.append(f'expected 40 display shadow objects, found {n_sh}')
